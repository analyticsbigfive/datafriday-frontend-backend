...
| [123](123_menu_components_cache_redis_invalidation_deploy.md) | Cache Redis composants (`GET /menu-components`, 60 s) : code invalide bien à la suppression, mais **vérifier/redéployer sur Render** (sinon composant supprimé servi jusqu'à 60 s) — **Ulrich** | ⚪ Diagnostiqué | 🟡 | Menu & recettes / Cache / Déploiement |
...
| [123-02](123_02_weezevent_organizationid_manquant_cron_echec_permanent.md) | Weezevent : `organizationId` manquant fait échouer le cron de sync indéfiniment (log error toutes les 10 min) + un PATCH pouvait le vider silencieusement | 🟡 Corrigé non testé | 🟠 | Intégrations & ventes |
| [125-01](125_01_event_timeline_groupby_placeholder_fuseau_500.md) | `getEventTimelineBatch` : 500 (42803) en prod — le correctif fuseau BUG-270 interpole `${spaceTimezone}` deux fois (SELECT + GROUP BY) → deux placeholders Prisma que Postgres ne peut pas apparier ; factorisé en `CROSS JOIN LATERAL` | 🟡 Corrigé non testé | 🔴 | Analyse & agrégation |
| [126-03](126_03_taxonomie_suppression_comptage_softdelete_non_filtre.md) | Gardes de suppression taxonomie : les comptages de blocage ne filtrent pas `deletedAt` → après soft-delete de tous les menu items, les catégories/types restent bloqués à tort (menu items + prix marché) — **à redéployer sur Render**, **Ulrich** | 🟡 Corrigé non déployé | 🟠 | Menu & recettes / Achats & référentiels |
| [127-02](127_02_spacemenus_combo_item_no_recipe_indisponible.md) | `getItemsWithAvailabilityForSpace` ignorait la relation `MenuItemCombo` (`comboChildren` ni sélectionnée ni comptée) → un menu item composé uniquement d'articles combo avait `hasRecipe=false` → affiché "No recipe"/indisponible dans Space Menus alors qu'il a bien une recette. Fix additif (isolé aux items avec `comboChildren`, zéro impact sur le reste du catalogue) : résolution récursive calquée sur `collectComponentIssues` | 🟡 Corrigé non déployé | 🔴 | Menu & recettes / Space Menus |
| [128-02](128_02_redis_cache_jamais_invalide_double_prefixe.md) | `invalidateCache()` (menu-components + ingredients) passait un pattern déjà préfixé `datafriday:` à `deletePattern`, qui préfixe lui-même en interne → double préfixe ne matchant jamais aucune clé réelle → cache liste (TTL 60s) jamais invalidé après create/update/delete. Fix : retrait du préfixe manuel redondant | 🟢 Corrigé | 🟠 | Menu & recettes / Achats & référentiels |
| [129-02](129_02_composants_supplier_marketprice_non_inclus_champ_denormalise_vide.md) | Composants : `marketPrice` non inclus dans la relation ingredient des sous-composants (colonne Supplier front toujours vide) + `MarketPrice.supplier` (dénormalisé) vide sur des lignes réelles malgré `supplierId` valide (données historiques désynchronisées). Fix : include scopé `marketPrice.supplier/supplierId/supplierRel.name` sur les 2 services concernés | 🟢 Corrigé | 🟠 | Menu & recettes / Achats & référentiels |
| [130-01](130_01_shop_details_rpc_costmap_scan_transactions_index_manquant.md) | RPC `get_space_shop_details` : 14,7 s par appel — scan heap de 185k lignes faute d'index couvrant sur `WeezeventTransaction` (le planner ignorait le composite 3 colonnes, `eventId` restait en heap) + costMap construite en scannant `WeezeventTransactionItem` alors qu'elle ne dépend que des mappings ⋈ `MenuItem`. Fix : index 4 colonnes `(tenantId,locationId,status,eventId)` (index-only) + costMap depuis les mappings + single-flight `RedisService.getOrSet` → **0,65 s** mesuré sur datafriday-dev. Volet front (double appel sérialisé) : web BUG-323-01 | 🟡 Corrigé non déployé | 🔴 | Analyse & agrégation |
| [130-01](130_01_geteventtimelinebatch_max_ecrase_merchants.md) | `getEventTimelineBatch` : régression du commit perf `8bd792a` — `MAX(...)` à un seul niveau en écrasant `weezeventMerchantId` dans le GROUP BY → deux caisses vendant le même article à la même minute ne comptent qu'une fois, timeline réelle aplatie en plateau constant. Fix : agrégation deux niveaux (MAX au grain merchant pour dédupliquer les jumelles writer-tag, puis SUM au grain affichage) | 🟡 Corrigé non déployé | 🔴 | Analyse & agrégation |
| [131-01](131_01_ingredients_select_omet_purchaseunitconversion.md) | `GET /ingredients` : le select du MarketPrice niché (`marketPriceSelectNoImage`) omettait `purchaseUnitConversion` → le front traitait la conversion unité d'achat comme 1 (« 405 packs de 1 kg » au lieu de 9 pour la viande tranchée à 0,02 kg/pièce, réunion Bertrand 19/08). Fix : champ ajouté au select. Fiche miroir web BUG-342-01 | 🟡 Corrigé non déployé | 🟠 | Achats & référentiels / Stock |
| [132-01](132_01_permission_dediee_preinventorypredicted.md) | Réunion Bertrand 19/08 : chip « Besoin prédit » du Pre-event Inventory réservé admins + directeurs de site — nouvelle permission `front.fb.preInventoryPredicted` (catalogue + rôle Directeur de site, propagation automatique aux tenants, pas de SQL). Chef exécutif exclu volontairement. Gating d'affichage front (fiche web BUG-343-01) | 🟡 Corrigé non déployé | 🟡 | RBAC / Stock |
| [133-02](133_02_loadrecipecontext_mpbyname_population_readyforsale_yes_obsolete.md) | Balayage complet Logistique (Ulrich) : `loadRecipeContext` peuplait la résolution du conditionnement par nom (`mpByName`) à partir des items `readyForSale='Yes'` mono-ingrédient — population devenue obsolète depuis BUG-048 (ces items ne consomment plus jamais `mpByName`, ils retournent tôt). Le vrai consommateur (boucle ingrédients, items `readyForSale='No'`/combo — le chemin d'explosion principal) ne recevait donc jamais de résolution par nom : packagingType silencieusement null sur potentiellement tout le catalogue. Fix : population recalée sur la bonne garde + repli niveaux orphelins de `getStock` résolu par `marketPriceId` déjà connu + `orderBy` déterministe sur les 3 résolutions par nom (itemName non unique) | 🟡 Corrigé non testé | 🟠 | Stock (Logistics) |
| [134-01](134_01_attendus_inventaire_source_etat_logistic.md) | Pre-event Inventory sans aucune quantité attendue (bandeau « [Simulé] … ») : l'ancre prenait LE dernier événement passé — un event simulé (outil QA) jamais compté — sans remonter au dernier match compté ; fenêtre de rejeu depuis la SAISIE du comptage (livraisons avalées en saisie tardive) ; permission `preInventoryPredicted` jamais propagée aux tenants existants. DÉCISION JLH 2026-08-20 : attendu (pre ET post) = **état Logistic au chargement de l'écran** (chemin partagé avec `getStock` — les deux écrans ne peuvent plus diverger), remplace le rejeu snapshot+mouvements ; events simulés exclus du repli d'inventaire ; sync du catalogue RBAC au boot (`RbacCatalogSyncService`). Corrige au passage la fausse promesse de propagation de BUG-132-01 | 🟡 Corrigé non déployé | 🔴 | Stock / Inventaire / RBAC |
| [135-01](135_01_transactions_count_compte_des_lignes.md) | `SpaceRevenueMinuteAgg."transactionsCount"` comptait des **LIGNES de vente** : `aggregation.service.ts:477` écrivait `COUNT(ti."id")` là où l'autre écrivain de la même colonne (`space-aggregation.service.ts:174`) écrivait déjà `COUNT(DISTINCT t.id)`. Sur « Le Mans-Brest » 22/08/2026 : **13 925 publiés pour 5 721 tickets réels**, panier moyen 4,71 € au lieu de 11,46 €. Propagé à `Event.transactionCount`/`avgSpendPerTx` et au RPC `get_space_shop_details`. Fix : `COUNT(DISTINCT t."id")` (additif à ce grain : une transaction n'a qu'une date, une location, un merchant). **Recalcul historique par SQL ensembliste appliqué à la main** (`20260824120000_fix_transactions_count`, validé à blanc : 13 925 → 5 721) — ne rattache que 282 250 lignes sur 547 954, les 109 events restants demandent `POST /aggregation/process-events`. Divergence merchantId/locationId + `status='V'` entre les deux écrivains : fiche à ouvrir. Fiche miroir web BUG-354-01 | 🟡 Corrigé non déployé | 🔴 | Analyse & agrégation |
| [135-02](135_02_migrate_dev_shadow_db_externalmerch_manquant.md) | `prisma migrate dev` casse systématiquement (shadow DB) : `ExternalMerch` déclaré dans le schema depuis le commit initial mais jamais créé par aucune migration versionnée (0_init ne le crée pas). Contourné au cas par cas (migration écrite à la main + `migrate deploy`/`db execute`+`migrate resolve`), jamais réparé à la racine. Drift secondaire observé sur la base de dev du 2026-08-24 (renames d'index, DROP DEFAULT non trackés) — à ne pas confondre avec le gel intentionnel déjà documenté de `MenuItem.spaceIds/spacePrices` et `WeezeventIntegration.clientId/clientSecret/organizationId`. | ⚪ Diagnostiqué | 🟠 | Technique |
| [136-01](136_01_scope_ventes_une_seule_integration.md) | `resolveEventSalesScope` faisait un `locationSpaceMapping.findFirst` **sans `orderBy`** → une seule intégration retenue au hasard, et `AND t."integrationId" = <celle-là>` vidait `getTransactionBasketsBatch` (dernier lecteur des transactions brutes, seul à appliquer ce filtre) de toutes les ventes de l'autre intégration. 7 espaces sur 31 concernés. Symptôme : donut « catégories par transaction » à 0 pendant que le reste de la page, qui lit la pré-agrégat, affichait le CA de l'event. Fix : `findMany` + `= ANY(...)` + `shopScopeClause` sur `integrationIds.length` ; sûreté vérifiée en base (aucun event ne mélange deux intégrations). Ajoute `minuteLocal` aux lignes paniers (le curseur horaire daté de BUG-351-01 les vidait sur un event franchissant minuit). `getLiveStatus` garde un `findFirst` — même classe de défaut, à qualifier. Fiche miroir web BUG-355-01 | 🟡 Corrigé non déployé | 🔴 | Analyse & agrégation / Intégrations & ventes |
| [137-01](137_01_ventes_non_mappees_comptees_endpoint_informatif.md) | Ventes non mappées : décision JLH 2026-08-24 **avec aller-retour tracé** — exclusion demandée puis implémentée (filtres `wpm."menuItemId" IS NOT NULL` + scope PdV strict), puis **écartée le jour même** après mesure d'impact (Adidas Arena 100 % / 4,09 M€, Auxerre/St-Étienne 100 % ; 0 ligne non mappée sur les intégrations actives Le Mans FC). État final : **comptées**, affichées « Non mappées », aucun filtre de mapping dans les lectures. Nouvel endpoint informatif `GET /spaces/:id/analyse-unmapped` (mêmes fenêtres/scope/prédicats que timeline+paniers) pour le bandeau de la page. Cause unique mesurée : produit importé mais jamais mappé à l'étape 3 du wizard (0 productId NULL sur 786 882 lignes). NE PAS ré-introduire l'exclusion en croyant appliquer « la règle » | 🟡 Corrigé non déployé | 🟠 | Analyse & agrégation / Intégrations & ventes |
| [138-01](138_01_sync_page_unique_500_curseur_horloge.md) | Sync Weezevent : **transactions perdues définitivement lors des pics de débit**. 81 tx / 892 € HT jamais importées sur Le Mans-Brest (pic mi-temps ~1 437 tx/30 min), full sync hebdo inopérante (500 importées puis stop, mesuré). Trois défauts composés, vérifiés à la seconde près : (1) réponse API tableau → normalisation `total_pages: 1` → `hasMore` false après UNE page (`weezevent-client.service.ts:79-88`) ; (2) cron demande `batchSize: 500` alors que le service documente per_page max 100 → page unique de 500 = le « plafond » observé deux fois ; (3) curseur = `new Date()` − 5 min (horloge murale, pas max importé) → le reliquat et les ventes hors-ligne en retard (18 min mesurées) sortent de la fenêtre, plus jamais retentés. Fix : pagination par `data.length === perPage` + batchSize ≤ 100, curseur au MAX importé (ou par ID), chevauchement 30 min, garde-fou de contiguïté des IDs. Rattrapage : RUNBOOK 2026-08-24 §2 | ⚪ Diagnostiqué | 🔴 | Intégrations & ventes |
| [139-01](139_01_resync_manuelle_fromdate_ignore.md) | **Aucun chemin de resync ne rattrape une fenêtre passée.** (a) Le bouton « Re-synchroniser » poste `{type, integrationId}` **sans aucune borne** (`DataIntegrationView.vue:1679`) → incrémental pur, fenêtre ≈ maintenant : il ne regarde que vers l'avant, et le dialogue affiche `res.count` (total en base, 6 721) au lieu de `itemsCreated` (0) — un rattrapage nul s'affiche comme un succès. (b) Le curl daté du runbook échoue autrement : `syncTransactionsIncremental` ignore `options.updatedSince` dès que `useIncremental` est vrai (`weezevent-incremental-sync.service.ts:260-262`), le `fromDate` demandé est écrasé par `lastSyncedAt − 5 min` alors que `toDate` est honoré (`:263`) → `created_at__gte` (jour J) **>** `created_at__lte` (22/08) : ensemble vide par construction. `autoFullForType` ne sauve pas le cas (ne force `full` que si la table est vide). C'est pourquoi les 81 transactions de Le Mans-Brest manquent encore. Contournement : curl + `"full": true` + fenêtres < 500 tx. Fix : `updatedSince` prioritaire sur le curseur, 400 si `fromDate > toDate`, remonter la fenêtre appliquée, et faire passer les rattrapages datés par le job chunké | ⚪ Diagnostiqué | 🔴 | Intégrations & ventes |
| [140-01](140_01_digifood_csv_heures_locales_stockees_utc_decalage_2h.md) | Timeline Analyse **+2 h sur les events importés par CSV Digifood** (La Beaujoire Nantes / Nantes-Rodez, 4 204 tx : source 11:58→16:20, affiché 13:58→18:20). `parseCsvDate` finissait par `new Date("…T11:58")` — horodatage naïf interprété dans le fuseau du PROCESS : sur Render (UTC), l'heure murale Paris était stockée telle quelle comme UTC, puis la conversion de lecture (BUG-270/125-01) rajoutait +2 h. Symptôme dépendant de l'environnement (poste dev en Europe/Paris = correct). Fix : horodatage à fuseau explicite parsé direct ; horodatage naïf = heure murale `CSV_NAIVE_TIMEZONE='Europe/Paris'` convertie en vrai UTC via `utcOffsetMinutes` (event-window.util), indépendamment du process. Réparation existant : réimport CSV (l'upsert réécrit `transactionDate`) + re-agrégation | 🟡 Corrigé non testé | 🔴 | Intégrations & ventes / Analyse & agrégation |
| [141-01](141_01_digifood_import_transactions_exclues_matching_eventid_site.md) | **Analyse vide après un import CSV Digifood** (« Aucun détail de vente disponible », réunion 24/08) : les tx importées sont liées au SalesEvent « site » (`upsertSiteAsEvent`) — ni `eventId IS NULL`, ni conteneur (span observé < 2 jours au démarrage) → exclues du matching fenêtre de `executeProcessEvents` → zéro ligne d'agrégat écrite. Corrigé par Ulrich (develop `ebea78d`) : tout SalesEvent `metadata.provider='digifood'` est classé conteneur d'office. Reste : réimporter (BUG-140-01) PUIS re-agréger, vérifier La Beaujoire/Nantes-Rodez | 🟢 Corrigé | 🔴 | Analyse & agrégation / Intégrations & ventes |
| [142-01](142_01_deux_events_meme_jour_fenetres_chevauchantes_sans_heure_fin.md) | **Deux events le même jour au même stade → données du mauvais match** (06/09 prod : SFP Montauban affiché au lieu de PFC Dior). La désambiguïsation même-jour existe (BUG-339-02 : fenêtre = minuit→heure de fin ; le match du soir démarre à la fin de celui d'après-midi) mais repose sur `eventEndTime`/`sessions.showTime` : un event sans heure exploitable retombe sur le jour ENTIER → recouvrement total, attribution croisée, silencieuse. Fix probable = saisie des heures des deux events + re-agrégation ; vérifier aussi que la prod exécute BUG-339-02 ; durcissement optionnel : warn sur recouvrement même-jour. **Suite 24-25/08 : cause racine chiffrée en base (145-01), règle métier décidée par Bertrand (146-01)** | ⚪ Diagnostiqué | 🔴 | Analyse & agrégation |
| [143-01](143_01_endpoints_batch_analyse_sans_cache_redis.md) | **Endpoints batch Analyse sans cache Redis** : `event-timeline`/`transaction-baskets` recalculaient 7-27 s de SQL par paquet à CHAQUE visite (Jean Bouin : ~110 s/page) alors qu'un event passé est immuable. Fix : cache Redis PAR EVENT (`spaces:evtimeline:…`/`spaces:baskets:…`), TTL 6 h passé / 60 s live (fenêtres exposées par `resolveEventSalesScope.windows`), invalidation dans `invalidateSpaceCache` ET à la complétion d'`executeProcessEvents` (constante partagée `eventBatchCachePatterns`) ; `analyse-unmapped` volontairement NON caché (BUG-137-01) ; + CORS `maxAge: 86400` (preflight ~180-380 ms par requête). Moitié front : chargement progressif, fiche frontend 363-01 | 🟡 Corrigé non testé | 🔴 | Analyse & agrégation / Performance |
| [144-01](144_01_oom_render_analyse_jean_bouin_concurrence_batch.md) | **OOM Render sur la page Analyse : « Instance restarted » sans stacktrace** (SIGKILL cgroup, plan free 512 Mo) pendant un chargement Jean Bouin à froid ; requêtes batch 20-38 s (HAR 24/08), ~164 Mo de JSON servis au montage. Cause : jusqu'à 8 requêtes SQL lourdes simultanées (4 chargeurs front × pool 2 PAR endpoint, récidive 357-01/361-01), aucune protection serveur, CRON dans le même process. Correctifs planifiés (plan 25/08) : file globale front concurrence 2 (fiche frontend 364-01), sémaphore serveur (2 en vol, file 32, 60 s → 503) autour des 3 méthodes batch, `resolveEventSalesScope` allégé + cache 60 s, cache `analyse-unmapped` (remplace la décision 137-01), et surtout réponses de montage sans grain minute (~10-20 Ko/event) | 🟡 Corrigé non testé (25/08 : garde-fous + timeline summary livrés ; paniers summary différés) | 🔴 | Analyse & agrégation / Performance |
| [145-01](145_01_attribution_fenetre_double_comptage_agregats.md) | **Attribution par fenêtre horaire : 80 343,07 € comptés DEUX FOIS, CA des jours à double affiche faux** — vérifié en base de prod (réconciliation des 77 events Jean Bouin, 24/08). `Event.revenue` = SUM(minuteAgg espace) au centime sur 77/77 (rollup frais, pas périmé) et somme = carte accueil au centime — mais l'attribution amont est fausse les jours à 2 matchs : 06/09 PFC-Dijon fem 35 301,39 € ⊂ SFP-Montauban 89 182,24 € (réel par tag conteneur : 1 975,38 € / 87 206,80 €) ; 06/12 Le Havre fem 45 041,68 € ⊂ Cardiff 67 442,89 € (réel : 478,49 € / 67 001,61 €). Aggravants : Montauban saisi `eventDate` 20/09 > `eventEndDate` 06/09 (aucune validation croisée des dates n'existe) → 0 € dans l'Analyse ; agrégats dupliqués sous 2 espaces SUPPRIMÉS (anciens espaces par club — qui, eux, séparaient juste). Correctif = règle Bertrand (146-01) + 3 SQL préparés (dates Montauban, backfill lien club, purge orphelins) | ⚪ Diagnostiqué | 🔴 | Analyse & agrégation |
| [146-01](146_01_attribution_integration_fenetre_portes.md) | **Règle d'attribution décidée par Bertrand (mail 25/08 + slide « Transactions prises en compte par Event »)** : une vente compte pour un match si elle vient de la data integration du club ET tombe dans [ouverture des portes, heure de fin] (minuit franchi OK) ; fenêtre seule en repli des sources sans tag (CSV Digifood). État du code : fenêtre portes→fin déjà en place dans l'agrégation (mode `range` BUG-329-02, −2 h/+2 h) mais PAS dans l'Analyse (minuit→fin) ; vérification du club absente partout (`eventLinkClause` laisse toute tx conteneur entrer dans la fenêtre de n'importe quel event). Implémentation : mode `container-range` (`t.eventId = conteneur` + fenêtre portes), backfill `Event.weezeventEventId` = conteneur du club (77 events, zéro migration), Analyse alignée (portes→fin + `eventStartDate ?? eventDate` + filtre conteneur), KPIs Analyse ← `Event.revenue`. **Volet horaire invalidé le soir même par 147-01 (relecture de la slide) — volet tag conteneur conservé** | 🟡 Corrigé non testé (code 25/08 ; en attente SQL + ré-agrégation, cf. INSTRUCTIONS_BACKEND_2026-08-25.md) | 🔴 | Analyse & agrégation |
| [147-01](147_01_attribution_fenetre_minuit_fin_slide.md) | **La slide « Transactions prises en compte par Event » dit « minuit → heure de fin », pas « portes ±2 h »** — les boîtes « Ouverture des portes » y sont des repères, la bande démarre à 00h00 et le match suivant démarre à la fin du précédent (erreur de lecture de 146-01). Symptôme : timeline EventPredict tronquée (12:53–18:42), ventes minuit→portes−2h jamais rattachées. Fix : règle unique `resolveEventTransactionWindow` (event-window.util) partagée lecteur (`resolveEventSalesScope`) / writer (`resolveEventWindow`) — minuit LOCAL → fin déclarée (repli journée pleine, règle Ulrich 360-02) + frontière au voisin, désormais AUSSI côté writer (anti-double-comptage 145-01 pour le repli CSV sans tag, voisin hors batch chargé en incrémental). Tag conteneur 146-01 inchangé ; marges ±2 h retirées de l'attribution (restent au staffing) | 🟡 Corrigé non testé (code 25/08 ; même train que 146-01 : SQL + ré-agrégation + purge Redis evtimeline) | 🔴 | Analyse & agrégation |
| [377-02](377_02_cache_ttl_catalogue_menu_ingredients_marketprice.md) | TTL cache catalogue trop court (60 s) sur menu-items / menu-components, rechargés à chaque montage du nouveau Live ; ingrédients laissés tels quels (market-price non invalidé, décision du 2026-09-03) | 🟡 Corrigé partiellement | 🟡 | Performance / Cache |
| [379-02](379_02_live_agregation_jamais_pendant_le_match_polling_aveugle_webhook_jamais_branche.md) | **Live : le calcul pour l'écran Live ne se lance jamais pendant un match, il faut cliquer "re-run" toutes les 10 min** (remontée Bertrand 12/09, AJA-Nice et PFC-Lyon). Prouvé en base de prod : la sync 10 min tourne bien (ventes en base à 16:30, 16:40 ... 21:10) mais le filet de sécurité 5 min de BUG-109 ne tourne que de 00:00 à 02:55 : sa fenêtre "live" = `eventEndDate` (minuit) + 3h, `eventEndTime` (23:50) ignoré, alors que `resolveEventTransactionWindow` existe. Le webhook Weezevent n'a jamais été mis en service (0 reçu, tous tenants) et le récepteur est incompatible avec le vrai format WeezPay (`id`/`values` à la racine vs DTO `data`, `forbidNonWhitelisted` → 400, signature sur body re-sérialisé). Plan : lot A polling adaptatif (30 min hors match, 10 s pendant un match sans webhook, 2 min avec webhook sain) + agrégation enchaînée coalescée + fenêtre partagée ; lot B webhook réel (DTO, raw body, UI Data Integration, questions au contact Weezevent) ; lot C heartbeat / alerte / exception handlers worker. Implémenté le 14/09 sur `fix/live-realtime-379` (scheduler 10 s en repli, agrégation par minute, récepteur WeezPay raw body, bloc webhook Weezevent dans Data Integration) ; reste l'enregistrement chez Weezevent et les décisions Render | 🟡 Corrigé non déployé | 🔴 | Live / Intégrations & ventes / Analyse & agrégation |
| [383-02](383_02_inventaire_articles_une_seule_configuration_push_logistic_propositions.md) | Pre/Post-event Inventory : articles à compter limités à LA configuration de l'event (39 events Jean Bouin liés à "Plan Max", 1 article) ; et en post-event les propositions reportées du pre-event (non recomptées) étaient poussées vers Logistic comme un comptage. Règles Bertrand 15/09 : PdV de la configuration, articles = union de toutes les configurations de l'espace (`?itemsScope=space`, invité PIN idem), Logistic mis à jour uniquement pour ce qui est compté (`isCounted`) | 🟡 Corrigé non déployé | 🔴 | Inventaire / Logistic |
| [384-02](384_02_agregats_pollues_par_les_integrations_des_autres_espaces_du_tenant.md) | **Le Mans-Brest : 112 438 € en KPI event sélectionné vs 66 457 € dans le graphe par PdV.** Le writer d'agrégats en mode fenêtre `range`, lancé sans `integrationId` (backfill BUG-352 du 08/09), rattachait toutes les ventes du tenant du même jour à l'event (FC Nantes Digifood dans l'espace Le Mans) ; la purge scopée BUG-317-02 ne retirait jamais ce résidu et le rollup `Event.revenue` le sommait. 176 events / 7,4 M€ de CA parasite sur Eat Is Family. Fix : writer et rollup scopés aux intégrations mappées à l'espace (`SpaceIntegrationScopeService`), purge des lignes étrangères à chaque job, script de réparation `repair-foreign-integration-aggs.ts` | 🟡 Corrigé non déployé | 🔴 | Agrégation / Analyse |
