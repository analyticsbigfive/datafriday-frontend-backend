  ...
  methods: {
    ...
    async confirmDelete() {
      ...
      try {
        ...
        // (déjà chargée dans deleteTarget.categoryList, la même que la puce "N catégories" du
        // tableau) pour que l'utilisateur voie ce qui bloque avant d'aller les supprimer une à une.
        if (this.deleteTarget?.categoryList?.length > 0) {
          ...
          this.deleteActionLink = {
            label: `${this.t('productTypeList.viewLinkedCategories')} (${count})`,
