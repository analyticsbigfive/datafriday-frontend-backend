  ...
  <v-dialog :model-value="modelValue" @update:model-value="$emit('update:modelValue', $event)" max-width="460" persistent>
    <div class="cptd-card" :class="{ 'cptd-card--dark': isDark }">
      ...
      <div class="cptd-grad-header">
        <div class="cptd-grad-header__icon"><BadgePercent :size="20" color="white" /></div>
        <div class="cptd-grad-header__text">
          <div class="cptd-grad-header__title">{{ t('menuItemCreatePromotionTypeDialogTitle') }}</div>
          <div class="cptd-grad-header__sub">{{ t('menuItemCreatePromotionTypeDialogSubtitle') }}</div>
        </div>
      ...
      <div class="cptd-body">
        ...
        <div class="form-floating">
          <input
            ...
            @keyup.enter="submit"
          />
          <label for="cprtd-name">{{ t('menuItemCreatePromotionTypeDialogLabel') }}</label>
        </div>
      </div>
      <!-- Footer -->
      <div class="cptd-foot">
        ...
        <button class="cptd-btn cptd-btn--primary" :disabled="loading" @click="submit">
          <Check :size="14" />
          {{ loading ? '…' : t('menuItemCreatePromotionTypeDialogAdd') }}
        </button>
      </div>
    </div>
  </v-dialog>
...
<script>
import { AlertCircle, Check, BadgePercent, X } from 'lucide-vue-next';
import { useI18n } from '@/i18n/useI18n';
import { createPromotionType } from '@/api/endpoints/promotion-type.api';

export default {
  name: 'CreatePromotionTypeDialog',
  components: { AlertCircle, Check, BadgePercent, X },
  props: {
  ...
  methods: {
    ...
    async submit() {
      ...
      if (!trimmed) {
        this.error = this.t('menuItemCreatePromotionTypeDialogNameRequiredError');
        return;
      }
      this.loading = true;
      this.error = '';
      try {
        const response = await createPromotionType({ name: trimmed });
        const created = response?.data || response;
        const id = created?.id || created?._id;
        if (id) {
          await this.$store.dispatch('promotionTypes/addPromotionType', { ...created, id });
        }
        this.$emit('created', created?.id ? created : { id, name: trimmed });
        this.name = '';
        this.$emit('update:modelValue', false);
      } catch (e) {
