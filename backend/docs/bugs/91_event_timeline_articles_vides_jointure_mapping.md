# BUG-091 — `event-timeline` : item-level vide (0 article) alors que le shop-level affiche du CA — INNER JOIN mapping trop strict
...
- **Repo(s) concerné(s)** : les deux (`api-datafriday-staging` cause, `datafriday-web` symptôme + hardening — fiche miroir front 164)
...
(`useAnalyseItemRecords.js`) — indistinguable d'un « 0 article » légitime (fiche front 164).
...
chargement, avalé en `[]` (fiche front 164), et le cache par event du front fige le vide
...
fiche front 164) : warm-up `GET /health` au chargement + retry unique des GET en timeout si
...
HTTP 500/4xx → échec avalé côté front (fiche front 164) ; 200 avec `{ "<eventId>": [] }` →
...
- Fiche miroir front : `datafriday-web/docs/bugs/164_analyse_articles_echec_event_timeline_silencieux.md`
