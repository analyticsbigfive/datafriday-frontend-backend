-- BUG-128-01 : la construction de menuItemCostMap scannait "WeezeventTransactionItem"
-- (via un IN sur "WeezeventTransaction" filtré par locations + events paginés) alors que
-- la valeur produite ne dépend QUE du mapping produit→menu item et de "MenuItem"."totalCost" :
--   - clé   : wpm."menuItemId" (les lignes sans mapping étaient déjà éliminées par
--             mi."totalCost" IS NOT NULL — mi n'est joignable qu'à travers wpm) ;
--   - valeur: MIN(mi."totalCost").
-- Le passage par les transactions ne faisait que RESTREINDRE l'ensemble des clés aux items
-- réellement vendus — restriction inutile (le front ne consulte la map que par id d'item
-- vendu) et coûteuse : sur un gros espace, ce scan participait aux ~17-20 s de la RPC,
-- payées deux fois par chargement de la page Analyse (granular=0 puis granular=1,
-- cf. frontend BUG-323-01).
--
-- Fix : costMap construite directement depuis "WeezeventProductMapping" ⋈ "MenuItem",
-- scopée tenant. Surensemble des clés précédentes, valeurs identiques pour chaque clé
-- déjà présente. Reste dans la branche v_location_ids non-vide (shape inchangée pour un
-- espace sans location mappée : '{}').
--
-- Le reste du corps est identique à 20260731120000_shop_details_rpc_resolve_datafriday_event_ids.sql.
-- Rollback : ré-appliquer le corps de cette même migration 20260731120000.
...
BEGIN
  ...
  ELSE
    ...
    IF p_include_granular AND v_paginated_ids IS NOT NULL AND array_length(v_paginated_ids, 1) > 0 THEN
      ...
      FROM (
        SELECT
          ...
          -- BUG-123-01 : ev_direct prioritaire — la colonne contient en réalité l'id DataFriday.
          COALESCE(ev_direct.id, ev_df.id)                                     AS "datafridayEventId",
          COALESCE(ev_direct.name, ev_df.name, we.name, srma."weezeventEventId") AS "eventName",
        ...
        -- BUG-123-01 : l'agrégation écrit l'id "Event" DataFriday dans srma."weezeventEventId" ;
        -- on résout donc d'abord directement contre "Event".
        LEFT JOIN "Event" ev_direct
          ON ev_direct.id        = srma."weezeventEventId"
        ...
        LEFT JOIN "WeezeventEvent" we
         ...
         AND we."tenantId" = p_tenant_id
        -- BUG-021 : jointure par liaison explicite (au lieu de DATE(...) = DATE(...)) ;
        -- un Event dont le lien reste ambigu (weezeventEventId non résolu) ne matche plus rien
        -- ici, plutôt que de risquer une association silencieuse au mauvais WeezeventEvent.
    ...
    -- ── RTT G+H: events with attendance — all metadata including configurationId ─
    SELECT jsonb_agg(
      jsonb_build_object(
    ...
    -- BUG-021 : idem, jointure par liaison explicite plutôt que par DATE(...) = DATE(...).
    LEFT JOIN "Event" ev_df
    ...
    -- BUG-128-01 : directement depuis les mappings produit→menu item (scopé tenant),
    -- sans scanner les transactions — mêmes valeurs (MIN totalCost) pour toutes les
    -- clés que l'ancienne version produisait, clés supplémentaires inoffensives
    -- (items mappés jamais vendus : le front n'interroge la map que par id vendu).
  ...
  LEFT JOIN LATERAL ( -- Builder v2 : config = 1re adhésion de l'élément
    SELECT ce."configId"
