-- get_space_shop_details : UN seul passage sur SpaceRevenueMinuteAgg (2026-09-21).
-- Avant, le bloc shops (somme par PdV) et le bloc granular (somme PdV x event) relisaient chacun
-- toute la table minute de l'espace (244k lignes sur Adidas Arena, 1,2 a 1,8 s par passage a froid) :
-- ~4 s de RPC, 6 s a froid, sur le chemin critique du premier rendu de l'Analyse. Les deux blocs
-- se derivent maintenant d'une table temporaire _gran (location x event x element). Sortie
-- identique bloc par bloc (verifie en ensembles : shops, granular, events, costMap, meta) ;
-- seul l'ordre interne des tableaux jsonb_agg sans ORDER BY peut changer. 6 s -> 0,9 s a froid.
-- Appliquee en production le 2026-09-21 via psql.

CREATE OR REPLACE FUNCTION public.get_space_shop_details(p_space_id text, p_tenant_id text, p_page integer DEFAULT 1, p_limit integer DEFAULT 20, p_include_granular boolean DEFAULT false)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  WHERE se.type IN ('shop','fnb_food','fnb_beverages','fnb_bar','fnb_snack','fnb_icecream','merchshop')
    AND (
      ...
      EXISTS ( -- Builder v2
        ...
        WHERE z.id = se."zoneId" AND z."spaceId" = p_space_id
      )
    );
  ...
  -- Somme par PdV pour le bloc shops : dérivée de _gran (un seul passage), vide sans location.
  CREATE TEMP TABLE _shop_rev ON COMMIT DROP AS
    SELECT NULL::text AS "spaceElementId", NULL::numeric AS "revenueHt", NULL::numeric AS "transactionsCount", NULL::numeric AS "itemsCount"
    WHERE false;
  ...
  ELSE
    ...
    -- 2026-09-21 : UN seul passage sur SpaceRevenueMinuteAgg (244k lignes sur un gros espace,
    -- 1,2 à 1,8 s par passage à froid). Avant, la somme par PdV (bloc shops) et la somme
    -- PdV × event (granular) relisaient chacune toute la table : ~4 s de RPC. Les deux blocs
    -- se dérivent maintenant de cet agrégat intermédiaire.
    CREATE TEMP TABLE _gran ON COMMIT DROP AS
      SELECT
        srma."weezeventLocationId",
        srma."weezeventEventId",
        lsm."spaceElementId",
      ...
      GROUP BY srma."weezeventLocationId", srma."weezeventEventId", lsm."spaceElementId";

    INSERT INTO _shop_rev
      SELECT "spaceElementId", SUM("revenueHt"), SUM("transactionsCount"), SUM("itemsCount")
      FROM _gran
      WHERE "spaceElementId" = ANY(v_shop_ids_param)
    ...
    IF p_include_granular AND v_paginated_ids IS NOT NULL AND array_length(v_paginated_ids, 1) > 0 THEN
      ...
      FROM (
        SELECT
          (COALESCE(gr."spaceElementId", gr."weezeventLocationId") || '_' || gr."weezeventEventId") AS "elementId",
          gr."weezeventEventId"                                                AS "weezeventEventId",
          COALESCE(ev_direct.id, ev_df.id)                                     AS "datafridayEventId",
          COALESCE(ev_direct.name, ev_df.name, we.name, gr."weezeventEventId") AS "eventName",
          COALESCE(ev_direct."eventDate", ev_df."eventDate", we."startDate")   AS "eventDate",
          COALESCE(gr."spaceElementId", gr."weezeventLocationId")             AS "shopId",
          se.name                                                              AS "shopName",
          COALESCE(se.attributes::jsonb->>'originalType', se.type::text)      AS "shopType",
          se.attributes::jsonb->>'area'                                        AS "shopArea",
          SUM(gr."revenueHt")::numeric(12,2)                                   AS "revenueHt",
          SUM(gr."itemsCount")::integer                                        AS "itemsCount",
          SUM(gr."transactionsCount")::integer                                 AS "transactionsCount"
        FROM _gran gr
        LEFT JOIN "SpaceElement" se
          ON se.id = gr."spaceElementId"
        LEFT JOIN "Event" ev_direct
          ON ev_direct.id        = gr."weezeventEventId"
         AND ev_direct."tenantId" = p_tenant_id
         AND ev_direct."spaceId"  = p_space_id
        LEFT JOIN "WeezeventEvent" we
          ON we.id = gr."weezeventEventId"
         AND we."tenantId" = p_tenant_id
        LEFT JOIN "Event" ev_df
          ON ev_df."weezeventEventId" = we.id
         AND ev_df."tenantId" = p_tenant_id
         AND ev_df."spaceId"  = p_space_id
        GROUP BY
          gr."spaceElementId", gr."weezeventEventId", gr."weezeventLocationId",
          ev_direct.id, ev_direct.name, ev_direct."eventDate",
    ...
    SELECT jsonb_agg(
      jsonb_build_object(
        ...
        'configurationId',  ev."configurationId",
    ...
    LEFT JOIN "Event" ev_df
      ON ev_df."weezeventEventId" = ev.id
     AND ev_df."tenantId"  = p_tenant_id
     AND ev_df."spaceId"   = p_space_id
    ...
    -- BUG-128-01 : costMap directement depuis les mappings produit→menu item (scopé
    -- tenant), sans scanner les transactions — mêmes valeurs (MIN totalCost) pour
    -- toutes les clés que l'ancienne version produisait.
    SELECT jsonb_object_agg(key, value) INTO v_cost_map_json
    FROM (
      SELECT
        wpm."menuItemId"    AS key,
        MIN(mi."totalCost") AS value
      FROM "WeezeventProductMapping" wpm
      INNER JOIN "MenuItem" mi
        ON mi.id = wpm."menuItemId"
      WHERE wpm."tenantId" = p_tenant_id
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      'configId',             c.id,
      'configName',           c.name,
      'locationId',           COALESCE(f.id, fc.id, z.id),
      'locationName',         COALESCE(f.name, fc.name, z.name),
      'locationType',         CASE
                                WHEN f.id IS NOT NULL THEN 'floor'
                                WHEN fc.id IS NOT NULL THEN 'forecourt'
                                WHEN z.kind = 'FLOOR' THEN 'floor'
                                ELSE 'forecourt'
                              END,
  ...
  LEFT JOIN "Zone" z       ON z.id  = se."zoneId"       AND z."spaceId"   = p_space_id
  LEFT JOIN LATERAL (
    SELECT ce."configId"
    FROM "ConfigurationElement" ce
    WHERE ce."elementId" = se.id AND ce."configId" = ANY(v_config_ids)
    ORDER BY ce."createdAt" ASC
    LIMIT 1
  ) mce ON true
  INNER JOIN "Config" c
    ON c.id = COALESCE(f."configId", fc."configId", mce."configId")
  LEFT JOIN (
