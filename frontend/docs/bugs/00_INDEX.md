...
| [369-02](369_02_badge_integration_manuel_step4.md) | Step 4 : aucun repère fiable pour distinguer un event PFC d'un event SFP sur un space partagé (seul le nom saisi à la main, convention non garantie), et le menu "Mapper à un événement existant" proposait tous les events du space sans filtre (risque de rattacher par erreur un event de l'autre club). Fix : badge du nom réel de l'intégration (`Integration.name`, via `include` sur `Event.integration`) affiché à côté du nom, cliquable pour reclasser manuellement (`updateEvent(id, {integrationId})`) ; menu de mapping filtré aux events non-taggués ou déjà de l'intégration courante | 🟢 Corrigé | 🟡 | Intégrations & ventes |
