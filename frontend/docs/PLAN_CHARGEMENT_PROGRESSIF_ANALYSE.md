...
Fiche associée aux lots A + D + B : [BUG-226](bugs/226_chargement_analyse_dedup_catalogues_et_phase2_en_vagues.md).
...
> Voir [BUG-227](bugs/227_shop_items_photo_base64_dupliquee_par_pdv.md).
...
3. **Union toutes-configs** — `loadAllConfigsShopContext` = `N configs × buildConfigShopEntry`
   (pool 3). ✅ **Déjà traité** par [BUG-225](bugs/225_analyse_predict_config_par_defaut_et_dedup_contexte.md) :
...
- **Invalidation obligatoire** dans `resetBuilder2SubtypesCache()` — déjà appelé par `loadSpace`
  ...
  exactement pour cette raison que la dédup de BUG-225 s'est limitée au vol en cours.
