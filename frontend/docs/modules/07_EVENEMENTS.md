...
| 2 | **`Event.revenue`/`transactionCount`/`avgSpendPerTx`/`perCapita`/`calculatedAt` ne sont jamais écrits automatiquement** | `GET /analyse/kpis/events` (`analyse.service.ts:79-104`) additionne ces champs en supposant qu'ils sont peuplés → retourne structurellement `totalRevenue=0`/`avgRevenue=0`/`totalTransactions=0` pour tout tenant. Nuance : zéro composant frontend n'appelle `getAnalyseKpisEvents` (grep exhaustif) — ce n'est donc pas un widget visible cassé aujourd'hui, mais un pipeline mort-à-la-source si jamais câblé. **Mise à jour 2026-08-02** ([BUG-272-02](../bugs/272_02_eventformdrawer_revenue_hors_dto_400_property_should_not_exist.md)) : ces 4 champs sont désormais écrivables **manuellement** via `EventFormDrawer.vue` → `PATCH/POST /events` (le DTO les ignorait auparavant, provoquant un 400 dès qu'un event mappé — donc avec un `revenue` non-null — était resauvegardé) ; toujours aucun calcul automatique côté backend en dehors du pipeline d'agrégation Weezevent | `events.service.ts` (écriture manuelle seulement, via DTO) ; `analyse.service.ts:79-104` ; `analyse.api.js:44-49` (zéro appelant) |
