# BUG-89 — `/analyse/*` : Swagger décrit des réponses qui n'existent pas, `?spaceId=` silencieusement ignoré

- **Statut** : 🟢 Corrigé (en code, non déployé)
- **Sévérité** : 🟠 Majeur (contrat API mensonger ; scoping espace inopérant)
- **Domaine** : Analyse & agrégation
- **Repo(s) concerné(s)** : les deux (backend = cause ; front = ex-consommateur, chaîne morte — voir `datafriday-web/docs/bugs/172_chaine_analyse_api_morte_supprimee.md`)
...
- `datafriday-web/docs/bugs/172_chaine_analyse_api_morte_supprimee.md`
