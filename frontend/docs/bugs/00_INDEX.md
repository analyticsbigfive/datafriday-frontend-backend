...
| [368-02](368_02_event_integrationid_mode_robuste_remplace_conteneur_saison.md) | Nouveau champ `Event.integrationId`, mode `integration-range` (prioritaire, sous exact) : matche directement `t.integrationId` + fenêtre calendaire, **jamais** `t.eventId` — élimine toute la détection de conteneur de saison (338/358/361/363/365-02) pour les events qui l'utilisent. Coexiste avec le mécanisme legacy BUG-146-01 (conteneur de saison, backfillé automatiquement vers le nouveau mode). Corrige aussi le trou de couverture multi-intégrations (`unregisteredDates`). **Migration écrite, PAS appliquée** — attend autorisation explicite (ADR-0002) | 🟡 Code prêt, migration en attente | 🟠 | Analyse & agrégation / Intégrations & ventes |
