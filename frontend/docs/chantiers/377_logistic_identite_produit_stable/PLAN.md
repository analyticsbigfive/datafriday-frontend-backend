...
- [x] Étape 5 — bascule d'écriture (portée retenue livrée, un point fermé en dette documentée) :
  ...
  - [x] `aggregateItems` : clé par `ref.id` au lieu de `ref.key` (nom) — deux articles homonymes restent deux entrées distinctes au lieu de fusionner dans le référentiel affiché (`/stock`, et par ricochet l'union storage `merged` qui réutilise la même clé). Vérifié type-safe ; aucun cas réel à démontrer, 0 homonyme actif en base.
  - [x] **Fermé, dette documentée** : `explodeSalesToConsumption` reste par nom, décision assumée — c'est un moteur de recette entièrement séparé (dupliqué à dessein de `itemRefsForMenuItem`, cf. sa propre docstring), qui ne sélectionne même pas les id ingrédient/composant aujourd'hui (seulement les noms). Le corriger demanderait de reconstruire tout le pipeline de dérivation des ventes en id-aware (select + explosion + propagation dans `getExpectedStockIndex`/`getStock`/`reset()`) — hors proportion avec le reste du chantier, et sans risque actuel avéré (0 collision homonyme active dans `StockMovement`, la consommation dérivée n'alimente que de l'affichage en lecture, jamais `StockLevel` directement). À reprendre dans un chantier dédié si un homonyme actif apparaît un jour.
