...
| [339-03](339_03_reco_logistique_inventaire_miroirs_fenetre_vente_non_alignes.md) | Suite de 339-02 : les deux « miroirs » documentés de la fenêtre de vente (`deriveEventConsumption` logistics, `getPostEventBaseline` inventory) calculaient toujours `eventDate → eventEndDate+1j` → la réco post-event de deux matchs consécutifs comptait la tranche minuit → fin du match précédent dans les DEUX matchs. Fix : logique 339-02 extraite en fonction pure partagée `computeEventSalesWindow` (event-window.util.ts), appelée par les 3 consommateurs ; repli jour entier si fenêtre nulle (la réco d'un event-conteneur reste calculable) ; 8 tests unitaires | 🟢 Corrigé | 🟠 | Logistique / Inventaire / Analyse & agrégation |
| [339-04](339_04_analyse_total_kpi_bascule_source_shop_vs_item.md) | Page Analyse : le CA TOTAL (bande KPI) change en fin de chargement (2,71M → 2,69M) — `kpiRecords` basculait silencieusement du shop-level (RPC shop-details, lien exact `weezeventEventId`, source de vérité) vers l'item-level (event-timeline, fenêtre de dates, conteneurs exclus, cap 50 events, formule différente) dès son arrivée. Fix front (option A validée JLH) : KPI épinglés sur le shop-level, item-level seulement si un filtre article/tranche horaire est actif (`hasItemOnlyFilters`). Suivi : alignement backend des deux sommes (option B) documenté dans la fiche, à valider séparément | 🟢 Corrigé | 🟠 | Analyse & agrégation |
