        ...
        <div class="ipd-panel" :class="{ 'ipd-panel--dark': isDark }">
          ...
          <div class="ipd-header">
            ...
            <div class="ipd-header__titles">
              <p class="ipd-header__title">{{ t('menuItemCreate.ingrTitle') }}</p>
              <p class="ipd-header__sub">{{ t('menuItemCreate.ingrSubtitle') }}</p>
            </div>
          ...
          <div v-if="error" class="ipd-error">{{ error }}</div>
          ...
          <div class="ipd-toolbar">
            ...
            <div class="ipd-search">
              ...
              <input
                ...
                :placeholder="t('menuItemCreate.ingrSearchPlaceholder')"
              />
          ...
          <div class="ipd-body">
            ...
            <!-- Empty -->
            <div v-else-if="!filteredGroups || !filteredGroups.length" class="ipd-empty">
              <div class="ipd-empty__icon"><Wheat :size="24" /></div>
              <p class="ipd-empty__text">{{ t('menuItemCreate.ingrNoData') }}</p>
            </div>
                ...
                <div v-if="isExpanded(group.itemName)" class="ipd-rows">
                  ...
                  >
                    ...
                    <div class="ipd-row__price">
                      <span class="ipd-row__price-val">{{ formatCurrency(row.costPerRecipeUnit) }}</span>
          ...
          <div class="ipd-footer">
            ...
            <div class="ipd-footer__actions">
              <button class="ipd-btn ipd-btn--cancel" :disabled="loading" @click="close">
              ...
              >
                ...
                {{ t('menuItemCreate.ingrAddBtn') }} ({{ selectedIds.length }})
              </button>
            </div>
          </div>
  ...
  methods: {
    ...
    addSelected() {
      ...
      const toAdd = picked.map(r => ({
        ...
        name:             r.ingredientName || r.itemName,
        type:             'Ingredient',
        category:         r.category,
        unit:             r.recipeUnit,
        quantity:         1,
    ...
    normalizeRow(marketPrice, ingredient) {
      const itemName = String(marketPrice?.itemName ?? marketPrice?.name ?? marketPrice?.marketPriceItemName ?? '').trim();
      const goodType = String(marketPrice?.goodType ?? marketPrice?.type ?? '').trim();
      // Vraie catégorie référentielle (table MarketPriceCategory, gérée dans
      // Settings > Configuration) — pas le texte libre legacy `category`, qui
      // divergeait de ce qui est réellement configuré.
      const category = String(marketPrice?.marketPriceCategory?.name ?? '').trim();
...
.ipd-rows__header-check { width: 17px; flex-shrink: 0; }
