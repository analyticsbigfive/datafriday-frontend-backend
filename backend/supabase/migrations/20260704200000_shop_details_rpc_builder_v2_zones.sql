-- Builder v2 : la RPC get_space_shop_details ne voyait que les shops v1
-- (Floor/Forecourt). Les éléments rattachés à une Zone (builder2 / migration v1→v2)
-- étaient absents du dashboard Analyse. Cette version ajoute :
--  - RTT B : les shops des Zones de l'espace ;
--  - shops list : LEFT JOIN Zone + config résolue via la 1re adhésion
--    ConfigurationElement (locationType 'floor'/'forecourt' conservé pour compat,
--    une zone FLOOR sort en 'floor', FORECOURT en 'forecourt', EXTERNAL en 'forecourt').
-- Basée sur 20260618000002 (dernière version).

-- Migration: expose WeezeventEvent.configurationId in get_space_shop_details events array
-- After G7 fix, WeezeventEvent.configurationId is stamped at Step 1 (location-space mapping).
-- The frontend Config filter uses this to restrict events per configuration.
