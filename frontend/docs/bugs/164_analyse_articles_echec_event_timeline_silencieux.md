# BUG-164 — Analyse : échec du batch `event-timeline` avalé → « Aucun article disponible » trompeur
...
- **Repo(s) concerné(s)** : `datafriday-web` (+ miroir cause backend :
  `api-datafriday-staging/docs/bugs/91_event_timeline_articles_vides_jointure_mapping.md`)
...
Le cas réel signalé (espace Auxerre) n'était **ni** la jointure backend (fiche 91 : le SQL déployé
...
Le vrai fix reste côté infra : keep-alive externe ou upgrade du plan Render (fiche back 91).
