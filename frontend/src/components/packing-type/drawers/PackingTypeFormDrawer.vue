        ...
        <div class="pkfd-panel" :class="{'pkfd-panel--dark': isDark}">
          ...
          <div class="pkfd-footer">
            ...
            <button class="pkfd-fbtn pkfd-fbtn--primary" :disabled="loading" @click="submit">
              <Save :size="14" />
              {{ loading ? t('packingTypeList.saving') : (mode === 'edit' ? t('packingTypeList.save') : t('packingTypeList.create')) }}
            </button>
          </div>
...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      if (!name) { this.error = this.t('packingTypeList.nameRequired'); return; }

      this.loading = true;
      try {
        const payload = { name };
        if (this.mode === 'edit') {
          if (!this.form.id) { this.error = this.t('packingTypeList.missingId'); return; }
        ...
        this.error = e?.response?.data?.message || e?.message || this.t('packingTypeList.saveError');
      } finally {
        this.loading = false;
      }
    },
