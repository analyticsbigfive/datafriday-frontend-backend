-- BUG-123-01 : les barres du mode Predict affichaient des UUID à la place des noms d'events.
--
-- Cause : l'agrégation (aggregation.service.ts, executeProcessEvents) insère l'id de
-- l'"Event" DataFriday (UUID) dans la colonne "SpaceRevenueMinuteAgg"."weezeventEventId"
-- (colonne mal nommée — elle contient des ids DataFriday, pas des ids WeezeventEvent).
-- La cascade de noms de get_space_shop_details (version 20260721210000) ne résolvait le nom
-- que via WeezeventEvent (we.id = srma."weezeventEventId" → ne matche jamais un UUID) puis
-- "Event"."weezeventEventId" via we.id (idem) → COALESCE retombait sur l'id brut.
-- Régression visible depuis BUG-021 : l'ancienne jointure par DATE masquait le problème.
--
-- Fix : joindre AUSSI "Event" directement sur srma."weezeventEventId" (ev_direct), prioritaire
-- dans les COALESCE de datafridayEventId / eventName / eventDate. Le repli id brut reste en
-- dernier recours (lignes réellement orphelines). Basée sur 20260721210000 (dernière version).
