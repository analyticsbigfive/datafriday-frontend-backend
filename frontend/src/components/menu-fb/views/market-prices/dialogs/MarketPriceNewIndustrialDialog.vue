  ...
  >
    <div class="mpnd" :class="{ 'mpnd--dark': isDark }">
      <div class="mpnd__header">
        ...
        <span class="mpnd__title">{{ locale === 'fr' ? 'Nouvel industriel' : 'New Industrial' }}</span>
      ...
      <div class="mpnd__body">
        ...
        <div class="mpnd-field-row">
          <label class="mpnd-field-label" for="mpnd-ind-name">
            {{ locale === 'fr' ? 'Nom' : 'Name' }} <span class="mpnd-required">*</span>
          </label>
          <input
            id="mpnd-ind-name"
            v-model="value"
...
<script>
// Dialog partagé de création d'un industriel (market price), extrait des drawers
// Create/EditSupplier. 100% frontend (endpoint existant `createIndustrial`). Le champ
// parent sélectionne PAR ID → on émet l'OBJET créé (avec `id`), le parent fait
// `form.industrialId = industrial.id`. Le refetch/ajout au store est fait ici.
import { Check, Factory, X } from 'lucide-vue-next';
import { useI18n } from '@/i18n/useI18n';
import { createIndustrial } from '@/api/endpoints/industrial.api';

export default {
  name: 'MarketPriceNewIndustrialDialog',
  components: { Check, Factory, X },
  props: {
