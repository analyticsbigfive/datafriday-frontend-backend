    ...
    async confirmBulkDelete() {
      ...
      for (const id of ids) {
        try {
          ...
          await this.$store.dispatch('eventCategories/removeEventCategory', id);
        } catch (e) {
          failed.push(id);
        }
        this.bulkProgress += 1;
...
.ecl--dark .bulk-bar { background:rgba(255,49,49,.1); border-color:rgba(255,49,49,.3); }
.ecl--dark .bulk-bar__clear { color:#94a3b8; }
.ecl--dark .bulk-bar__clear:hover { background:rgba(255,255,255,.06); color:#e2e8f0; }
