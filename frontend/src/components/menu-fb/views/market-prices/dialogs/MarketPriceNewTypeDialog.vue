  ...
  >
    <div class="mpnd" :class="{ 'mpnd--dark': isDark }">
      <div class="mpnd__header">
        ...
        <span class="mpnd__title">{{ locale === 'fr' ? 'Nouveau type de produit' : 'New Good Type' }}</span>
      ...
      <div class="mpnd__body">
        ...
        <div class="mpnd-field-row">
          <label class="mpnd-field-label" for="mpnd-type-name">
            {{ locale === 'fr' ? 'Nom du type' : 'Type name' }} <span class="mpnd-required">*</span>
          </label>
          <input
            id="mpnd-type-name"
            v-model="value"
...
<script>
// Dialog partagé de création d'un « Good Type » (market price), extrait des drawers
// Create/Edit qui le dupliquaient (cf. BUG-194 / dette dark mode). 100% frontend :
// appelle l'endpoint existant `createMarketPriceType`. Le parent gère la sélection du
// type créé dans son formulaire via `@created="name => ..."`.
import { Check, Shapes, X } from 'lucide-vue-next';
import { useI18n } from '@/i18n/useI18n';
import { createMarketPriceType } from '@/api/endpoints/market.price.api';

export default {
  name: 'MarketPriceNewTypeDialog',
  components: { Check, Shapes, X },
  props: {
