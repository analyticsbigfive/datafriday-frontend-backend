...
| [236](236_hr_crypto_randomuuid_contexte_non_securise.md) | HR : `crypto.randomUUID is not a function` sur IP LAN/HTTP (contexte non sécurisé) → création impossible ; fix helper `newId()` à repli. + **reconstruction propre de HR Suppliers** (CRUD liste/drawer+image/delete-dialog au pattern SuppliersListView, rendu dans le chrome DashboardView) + fix double-header `/hr` | 🟡 Corrigé non déployé | 🟠 | RH / Staffing |
