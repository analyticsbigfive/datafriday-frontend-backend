...
| [378-02](378_02_reco_post_event_integration_unique_predit_grain_article.md) | Réco post-event SFP-Perpignan : 325 lignes sans nom, Vendu = 0 partout, Diff -100 %. Trois causes : `deriveEventConsumption` ne lisait qu'UNE intégration par espace (Stade Jean Bouin = PFC + SFP, `findFirst`), le prédit était au grain menu item contre un comptage au grain ingrédient (clés jamais jointes, lignes orphelines), aucun périmètre sur pré-event/mouvements. Corrigé : `findMany` + `IN`, identité `(itemKind, itemRefId)` sur les lignes de consommation, prédit via la version par défaut au grain inventaire (`postEventPredicted.js`), périmètre compté, noms catalogue en repli, Miss € par kind (Q40 tranchée). | 🟡 Corrigé non déployé | 🔴 | Stock (Post-event) |
| [382-02](382_02_live_transactions_sommees_grain_article_panier_moyen_faux.md) | Live : la bande KPI et TX/MIN sommaient `transactionCount` du grain article (`SpaceRevenueMinuteItemAgg`), comptant un panier à N articles N fois. AJA-Nice 12/09, 18:33-20:20 : 3 721 affichés pour 1 299 tickets réels, panier 4,90 € au lieu de 14,03 €, TX/min 61 au lieu de ~12. Le fix BUG-354-01 (source paniers) n'avait été porté que sur l'Analyse. Fix : `utils/liveKpis.js`, tickets toujours lus depuis `transaction-baskets`, `transactionRecords` passé à `useMetricsCalculator` dans `LiveView.vue` | 🟡 Corrigé non déployé | 🔴 | Live |
| [385-02](385_02_reco_pre_event_ids_hors_menuitem_exclus.md) | Réco pre-event (Auxerre 6A) : l'inventaire compte « à 1 cran » sous des ids MarketPrice/MenuComponent (70 % des lignes en base), la feuille ne résolvait que MenuItem → lignes comptées exclues comme « orphelines » et remplacées par la valeur Logistic « (L) » sous l'id MenuItem homonyme ; aucun hint « Attendu » à l'écran pour ces lignes, conditionnement 1. Fix : jointure par nom vers tous les ids de catalogue, identités via `resolveItemKeysByIds`, dédoublonnage des alias, `itemKind` sur les lignes | 🟡 Corrigé non déployé | 🔴 | Stock |
| [386-02](386_02_paniers_paquet_trop_lourd_txmin_zero.md) | Analyse Jean Bouin, un event seul : carte **TX/MIN figée à 0,00/min** et panneau PdV à « 0.00 txn/min / Operating Minutes 0 », pendant que CA (55 196 €), per cap et marge restent justes. Attendu recalculé en prod : **29,92/min**. Deux défauts composés. (1) `transaction-baskets` est resté au grain minute (le summary de BUG-364-01 n'a porté que l'event-timeline) : **2 Mo de JSON par event, 42 Mo par paquet de 30 events** mesurés en prod, alors que le paquet est passé de 15 à 30 le 21/09 (`aa9276bd`) en le supposant « de quelques centaines de ko » → le paquet échoue sur l'instance 512 Mo. (2) `useTransactionBaskets` écrivait `[]` pour un event non livré : un ÉCHEC devenait « chargé, aucun ticket », `sourceState` retombait sur 'empty' et la carte publiait 0,00/min comme une valeur exacte, sans refetch possible de la session (le CA, lu sur le rollup `Event.revenue` depuis BUG-146-01, restait juste — d'où un écran qui ne signale rien). Fix : paquet paniers dédié (6), `failedIds` + tentatives bornées, état `'error'` distinct de `'empty'`, TX/MIN à « — » sans repli sur l'ancienne formule, cadence `null` (et non 0) pour un PdV absent de la source. Reste ouvert : grain summary des paniers | 🟡 Corrigé non déployé | 🔴 | Analyse & agrégation |
| [387-02](387_02_event_predict_activation_relit_tous_les_pdv.md) | Event Predict : cocher un article ou ouvrir un PDV (colonne de gauche) « rechargeait toute la page ». Deux gênes sous une phrase. (1) Perte du repère : l'onglet Configuration entier était remplacé par un skeleton donc démonté, scroll et PdV dépliés perdus — corrigé le 17/09 (`5fb0000f`, déployé). (2) L'attente : chaque activation purgeait les caches et rejouait `loadShopMenuAssignment()`, soit `/shops` puis `/space-menu/shop/:id/items` pour CHAQUE PdV de la config, 3 en vol (53 éléments PdV sur Jean Bouin) alors que le delta était connu. Fix : dérivation des 4 index extraite dans `utils/shopMenuAssignmentMaps` et partagée, patch en mémoire du seul PdV touché + du store de disponibilité (`patchEnabledForShop` au lieu d'une purge), branché sur le toggle unitaire, le Select All et l'ajout à tous les PdV ; repli sur le rechargement complet si l'état local ne permet pas le patch (remap, cache expiré) | 🟡 Corrigé non déployé | 🟠 | Event Predict |
