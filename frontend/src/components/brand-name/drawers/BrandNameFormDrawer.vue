        ...
        <div class="bnfd-panel" :class="{'bnfd-panel--dark': isDark}">
          ...
          <div class="bnfd-footer">
            ...
            <button class="bnfd-fbtn bnfd-fbtn--primary" :disabled="loading" @click="submit">
              <Save :size="14" />
              {{ loading ? t('brandNameList.saving') : (mode === 'edit' ? t('brandNameList.save') : t('brandNameList.create')) }}
            </button>
          </div>
...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      if (!name) { this.error = this.t('brandNameList.nameRequired'); return; }

      this.loading = true;
      try {
        const payload = { name };
        if (this.mode === 'edit') {
          if (!this.form.id) { this.error = this.t('brandNameList.missingId'); return; }
        ...
        this.error = e?.response?.data?.message || e?.message || this.t('brandNameList.saveError');
      } finally {
        this.loading = false;
      }
    },
