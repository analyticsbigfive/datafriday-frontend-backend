...
export class HrSuppliersController {
  ...
  update(@Param('id') id: string, @Body() dto: UpdateHrSupplierDto, @CurrentTenant() tenantId: string) {
    return this.service.updateSupplier(id, dto, tenantId);
  }

  @Delete(':id')
  @ApiOperation({ summary: 'Supprimer un fournisseur RH' })
  remove(@Param('id') id: string, @CurrentTenant() tenantId: string) {
    return this.service.removeSupplier(id, tenantId);
  }
}
