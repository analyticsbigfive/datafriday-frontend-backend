-- BUG-021 : get_space_shop_details joignait "Event" <-> "WeezeventEvent" par égalité de DATE
-- seule (DATE(ev_df."eventDate") = DATE(we."startDate")), sans discriminant si 2 events
-- Weezevent tombent le même jour calendaire sur le même espace — risque de confusion.
--
-- Fix : joint désormais par la liaison explicite "Event"."weezeventEventId" (ajoutée par la
-- migration Prisma 20260721210000_event_weezevent_link_and_webhook_per_integration), posée soit
-- automatiquement (EventWeezeventLinkService, quand exactement 1 Event + 1 WeezeventEvent
-- partagent tenant+date, sans ambiguïté), soit manuellement via l'API de désambiguïsation
-- (EventsController.resolveWeezeventLink) quand plusieurs candidats existaient le même jour.
--
-- Conséquence attendue : un Event dont le lien reste ambigu (non résolu) n'apparaît plus du tout
-- dans "shopGranularData"/"events" côté DataFriday (COALESCE retombe sur les champs
-- WeezeventEvent bruts, comme si aucun Event DataFriday n'existait) — préférable à l'ancien
-- comportement qui pouvait associer silencieusement le mauvais Event.
-- Basée sur 20260704200000 (dernière version).
