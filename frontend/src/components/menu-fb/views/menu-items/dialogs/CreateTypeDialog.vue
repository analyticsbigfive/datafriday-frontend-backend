    ...
    <div class="ctd-card">
      ...
      <div class="ctd-grad-header">
        ...
        <div class="ctd-grad-header__text">
          <div class="ctd-grad-header__title">{{ t('menuItemCreateTypeDialogTitle') }}</div>
          <div class="ctd-grad-header__sub">{{ t('menuItemCreateTypeDialogSubtitle') }}</div>
        </div>
      ...
      <div class="ctd-body">
        ...
        <div class="form-floating">
          <input
            ...
            @keyup.enter="submit"
          />
          <label for="ctd-name">{{ t('menuItemCreateTypeDialogLabel') }}</label>
        </div>
      </div>
      <!-- Footer -->
      <div class="ctd-foot">
        ...
        <button class="ctd-btn ctd-btn--primary" :disabled="loading" @click="submit">
          <Save :size="14" />
          {{ loading ? 'Création…' : t('menuItemCreateTypeDialogCreate') }}
        </button>
      </div>
    </div>
  </v-dialog>
...
import { createProductType } from '@/api/endpoints/product.api';
...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      this.error = '';
      try {
        const response = await createProductType({ name: trimmed });
