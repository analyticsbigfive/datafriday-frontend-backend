...
| 2 | ~~**Bug multi-onglets non corrigé**~~ — **corrigé le 2026-07-18**, voir [BUG-149](../bugs/149_auth_signed_out_rotation_deconnexion_multi_onglets.md). La décision est extraite dans `src/utils/authSessionEvent.js` (fonction pure, 11 tests) : sur événement sans session, la session persistée est relue et l'artefact de rotation distingué d'une vraie déconnexion. La propagation volontaire entre onglets est préservée. Deux défauts adjacents corrigés au passage : absence de garde de réentrance sur `getSessionOnce()`, et souscription `onAuthStateChange` jamais libérée (deux listeners empilables si `initialize` échoue puis est redispatché). | 🟡 Corrigé non déployé | `store/modules/auth.js`, `src/utils/authSessionEvent.js` | Scénario de rotation difficile à reproduire à la main (JWT 7 j) — voir la fiche pour les trois méthodes de forçage. |
...
| 4 | ~~Bypass démo (`?demo=1` / `localStorage.analyse_demo`)~~ — **corrigé le 2026-07-18**, voir [BUG-027](../bugs/27_bypass_demo_actif_sans_distinction_env.md). Retiré plutôt que restreint au dev : `utils/demoMode.js::isDemoMode()` retourne `false` en dur depuis un chantier antérieur, donc le flag ne servait plus aucune donnée mock — il n'accordait plus qu'un accès non authentifié. Test de non-régression ajouté (`tests/unit/routerGuards.spec.js`). | 🟢 Corrigé | `router/guards.js` | Écriture résiduelle du flag dans `EventPredictView.vue:3549`, désormais inerte — non nettoyée (domaine Prévision). |
| 5 | ~~`/predict-test` monté sans aucun guard~~ — **corrigé le 2026-07-18**, voir [BUG-028](../bugs/28_predict_test_sans_guard_auth.md). Route non montée quand `NODE_ENV === 'production'` ; accès libre conservé hors production, ce qui préserve l'usage du banc de test. | 🟢 Corrigé | `router/index.js` | Le chunk de `PredictTestView.vue` peut rester émis au build — la route, elle, n'est pas déclarée. |
...
| 9 | ~~`console.log` du JWT en clair à l'onboarding~~ — **corrigé le 2026-07-18**, voir [BUG-150](../bugs/150_auth_console_log_jwt_en_clair.md) | 🟢 Corrigé | `store/modules/auth.js:351` (avant correction) | — |
