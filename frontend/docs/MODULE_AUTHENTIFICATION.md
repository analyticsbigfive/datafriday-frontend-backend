...
| # | Bug | Sévérité |
|---|---|---|
| 1 | **`OrganizationsController` — faille cross-tenant.** 📄 Dossier technique complet : [`SECURITE_ORGANIZATIONS_CROSS_TENANT.md`](SECURITE_ORGANIZATIONS_CROSS_TENANT.md). Le correctif P0-1 a durci `/tenants` avec `SuperAdminGuard` ; `/organizations` expose les mêmes opérations sur le **même modèle `Tenant`** avec le seul `JwtDatabaseGuard`. Tout utilisateur authentifié de n'importe quel tenant peut lire, modifier (`plan` inclus) ou suspendre l'organisation d'un tiers. | 🔴 Critique |
