# BUG-96 — Inventaire : N+1 `GET /space-menu/shop/:shopId` alors qu'un batch existe (payload trop maigre)

- **Statut** : 🟢 Corrigé (en code, non déployé)
- **Sévérité** : 🟠 Majeur/perf (rendu initial Space Inventory)
- **Domaine** : Stock (Inventory) / Menu
- **Repo(s) concerné(s)** : les deux (canonique ici ; miroir `datafriday-web/docs/bugs/185_inventaire_adopte_batch_shop_items.md`)
...
- BUG-010 (N+1 predict/toolbox — l'autre moitié, event-timeline, corrigée côté front fiche 180)
- Miroir front : fiche 185
